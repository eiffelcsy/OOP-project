...
/**
 * Patient Schedules API Service
 * Provides read-only doctor schedule access for patient-facing flows.
 */
...
export const patientSchedulesApi = {
  /**
   * Get schedules for a doctor (patient-facing).
   * GET /api/patient/doctors/{doctorId}/schedules
   */
  async getSchedulesByDoctorId(doctorId: number): Promise<ScheduleResponse[]> {
    const response = await apiClient.get(`/api/patient/doctors/${doctorId}/schedules`)
